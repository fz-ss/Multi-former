...
def gene_pseudo_labels_1(feature_extractor,classifier, target_loader, output_dir, cfg):
    ...
    for i_iter in tqdm(range(cfg.MAX_ITERS_PSEUDO)):
        with torch.no_grad():
            ...
            pred_seg_trg,_,_ = classifier(feature_extractor(images.cuda(device)))  #1,16,80,160
...
def gene_pseudo_labels_2(feature_extractor,classifier, target_loader, output_dir, cfg):
    input_size_target = cfg.TRAIN.INPUT_SIZE_TARGET
    device = cfg.GPU_ID
    ...
    for i_iter in tqdm(range(cfg.MAX_ITERS_PSEUDO)):
        with torch.no_grad():
            ...
            pred_seg_trg,_,_ = classifier(feature_extractor(images.cuda(device)))  #1,7,80,160
            pred_seg_trg = interp_target(pred_seg_trg)
            pred_trg_main_1 = F.softmax(pred_seg_trg, dim=1)
            conf, target_2_1 = torch.max(pred_trg_main_1, 1)
...
def gene_pseudo_labels_3(feature_extractor,classifier, target_loader, output_dir, cfg):
    input_size_target = cfg.TRAIN.INPUT_SIZE_TARGET
    device = cfg.GPU_ID
...
def gene_pseudo_labels_4(feature_extractor,classifier, target_loader, output_dir, cfg):
    input_size_target = cfg.TRAIN.INPUT_SIZE_TARGET
    device = cfg.GPU_ID
...
def gene_pseudo_labels_5(feature_extractor,classifier, target_loader, output_dir, cfg):
    input_size_target = cfg.TRAIN.INPUT_SIZE_TARGET
    device = cfg.GPU_ID
