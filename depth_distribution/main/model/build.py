...
def build_classifier(cfg):
    ...
    backbone_size = (cfg.WEIGHTS.split('/')[-1]).split('_')[1]
    if backbone_size == 'small':
        classifier = UPerHead(
...
def build_adversarial_discriminator_cls(cfg):
    backbone_size = (cfg.WEIGHTS.split('/')[-1]).split('_')[1]
    if backbone_size == 'small':
        model_D = PixelDiscriminator(384, 384, num_classes=cfg.NUM_CLASSES) 
    else:
        model_D = PixelDiscriminator(512, 512, num_classes=cfg.NUM_CLASSES) 
    return model_D
